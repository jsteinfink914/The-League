<script>
  ...
  const loadData = async () => {
    ...
    try {
      ...
      if (!response.ok) throw new Error(`Player values could not be loaded (${response.status}).`);
      const result = Papa.parse(await response.text(), { header: true, skipEmptyLines: true });
      if (result.errors.length) throw new Error('Player values file could not be parsed.');
      const invalidRows = result.data.filter((row) =>
        !row.Year || !row.Name || !Number.isFinite(Number(row.Value)) || Number(row.Value) < 0
      );
      if (invalidRows.length) throw new Error('Player values file contains invalid or negative values.');
      data = result.data.map((row) => ({ ...row, Value: Number(row.Value) }));
      years = [...new Set(data.map(row => row.Year))]
        .filter(Boolean)
        .sort((a, b) => Number(b) - Number(a));
      selectedYear = years[0] || "";
      if (!selectedYear) throw new Error('No player value seasons are available.');
      updateData();
    } catch (error) {
      data = [];
      years = [];
  ...
  function updateTeamValues() {
    team1Values = team1Selections.map(name => {
      ...
      return player && Number.isFinite(player.Value) ? player.Value : null;
    });
    ...
    team1Total = team1Values.reduce((acc, value) => acc + (Number.isFinite(value) ? value : 0), 0);
    team2Total = team2Values.reduce((acc, value) => acc + (Number.isFinite(value) ? value : 0), 0);
  ...
  function handleSearchInput(index, team, event) {
    ...
    if (team === 1) {
      team1Selections[index] = input;
      team1Dropdowns[index] = { show: query.length > 0, filteredNames };
    } else {
      team2Selections[index] = input;
...
<div class="container">
  {#if loading}
    <p>Loading player values…</p>
  {:else if loadError}
    <div class="load-error" role="alert">
      {loadError} <button on:click={loadData}>Retry</button>
    </div>
  {/if}
    ...
    <div class="team">
      ...
      {#each Array(7).fill().map((_, i) => i) as index}
        <div class="team-entry">
          ...
          {/if}
          <span>Value: {team1Values[index] == null ? 'Unresolved' : `$${team1Values[index]}`}</span>
        </div>
      {/each}
    ...
    <div class="team">
      ...
      {#each Array(7).fill().map((_, i) => i) as index}
        <div class="team-entry">
          ...
          {/if}
          <span>Value: {team2Values[index] == null ? 'Unresolved' : `$${team2Values[index]}`}</span>
        </div>
      {/each}
      <div class="team-summary">
